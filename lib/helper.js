...
exports.createQueryString = function (args) {
  if (!args.u || args.p) {
    throw new Error('you must authenticate your db using -u and -p flags');
  }
  return `mongoimport --host localhost --db foobar -u ${args.u} -p ${args.p} --authenticationDatabase foobar --collection posts < ${filePath} db/mongo_insertion.json --jsonArray`;
};
